...
namespace HREngine.Bots
{
    public class ActionNormalizer
    {
                    ...
                    switch (aa.actionType)
                    {
                        ...
                        case actionEnum.playcard:
                            ...
                            else if (rndBeforeDamageAll && aa.card.card.type == CardDB.cardtype.SPELL && penman.DamageRandomDatabase.ContainsKey(aa.card.card.name))
                            {
                                ...
                                if (tmp.enemyHero.Hp != tmpPlOld.enemyHero.Hp) actIdDmg.Add(tmpPlOld.enemyHero.entityID, tmp.enemyHero.Hp - tmpPlOld.enemyHero.Hp);
                                if (tmp.ownHero.Hp != tmpPlOld.ownHero.Hp) actIdDmg.Add(tmpPlOld.ownHero.entityID, tmp.ownHero.Hp - tmpPlOld.ownHero.Hp);
                                bool found = false;
                ...
                foreach (Action a in reorderedActions)
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (actIdDmg.ContainsKey(tmpPf.enemyHero.entityID)) tmpPf.minionGetDamageOrHeal(tmpPf.enemyHero, actIdDmg[tmpPf.enemyHero.entityID]);
                            if (actIdDmg.ContainsKey(tmpPf.ownHero.entityID)) tmpPf.minionGetDamageOrHeal(tmpPf.ownHero, actIdDmg[tmpPf.ownHero.entityID]);
                            foreach (Minion m in tmpPf.enemyMinions)
                            {
                                if (actIdDmg.ContainsKey(m.entityID)) tmpPf.minionGetDamageOrHeal(m, actIdDmg[m.entityID]);
                            }
        ...
        private bool isActionPossible(Playfield p, Action a)
        {
            ...
            {
                actionFound = false;
                if (p.enemyHero.entityID == a.target.entityID) actionFound = true;
                else if (p.ownHero.entityID == a.target.entityID) actionFound = true;
                else
                {
