                        ...
                        if (trgts.Count == 0)
                        {
                            ...
                            if (usePenalityManager)
                            {
                                ...
                                if (cardplayPenality < 500)
                                {
                                    ...
                                    if (useplacement && ((hc.card.name == CardDB.cardName.direwolfalpha || hc.card.name == CardDB.cardName.flametonguetotem || hc.card.name == CardDB.cardName.defenderofargus || hc.card.name == CardDB.cardName.grimestreetprotector || hc.card.name == CardDB.cardName.voidterror) || (superplacement && hc.card.type == CardDB.cardtype.MOB && (hc.card.Charge || hc.card.tank))))
                                    {
                                        //int adding = 1;
                                        //int subbing = 0;
                                        ...
                                        //for (int placer = 0; placer < p.ownMinions.Count + 1 - subbing; placer++)
                                        //{
                                        ...
                                        if (p.ownMinions.Count == 2 && !hc.card.Charge && !hc.card.tank)
                                        {
                                            if ((hc.card.name == CardDB.cardName.defenderofargus || hc.card.name == CardDB.cardName.grimestreetprotector) && superplacement)
                                            {
                                                for (int placer = 1; placer <= p.ownMinions.Count + 1; placer++)
                                                {
        ...
        public bool didAttackOrderMatters(Playfield p)
        {
            ...
            foreach (Minion m in enemym)
            {
                ...
                switch (m.name)
                {
                    ...
                    case CardDB.cardName.savannahhighmane: spawnMinions = true; break;
                    ...
                    case CardDB.cardName.infestedwolf: spawnMinions = true; break;
                    ...
                    case CardDB.cardName.kindlygrandmother: spawnMinions = true; break;
                    //case CardDB.cardName: spawnMinions = true; break;
                    default: break;
                }
            }
            //if (spawnMinions) return true;
            ...
            foreach (Minion m in ownm)
            {
                ...
                //if (!m.silenced && (m.name == CardDB.cardName.tunneltrogg || m.name == CardDB.cardName.unboundelemental)) hasOverloadBuff = true;
                ...
                if (m.Ready && m.Angr >= 1)
                {
                    ...
                    switch (m.name)
                    {
                        ...
                        //case CardDB.cardName: spawnMinions = true; break;
                        //case CardDB.cardName.tunneltrogg: hasOverloadBuff = true; break;
                        //case CardDB.cardName.unboundelemental: hasOverloadBuff = true; break;
            ...
            if (hasBeasts >= 1 && hasTundraRhino) return true;
            if ((hasJuggler || hasCouncilman) && spawnMinions) return true;
            if ((hasTundraRhino || hasHyena) && hasDeathrattleBeast) return true;
