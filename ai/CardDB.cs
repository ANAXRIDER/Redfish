    ...
    public class CardDB
    {
        ...
        public class Card
        {
                        ...
                        if (p.playactions.Count > 0)
                        {
                            foreach (Action a in p.playactions)
                            {
                                if (a.actionType == actionEnum.playcard)
                                {
                                    ...
                                    {
                                        case cardName.tuskarrtotemic: retval -= p.anzOwnBranns + 1; break;
                                        default:
                                    ...
                                    if (a.card.card.type == CardDB.cardtype.SPELL)
                                    {
                                        foreach (Minion m in p.ownMinions)
                                        {
                                            if (m.name == CardDB.cardName.wickedwitchdoctor && !m.silenced && p.ownMinions.Count <= 6) retval--;
                                        }
                                    }
                                }
                                else if (a.actionType == actionEnum.useHeroPower)
                                {
                                    ...
                                    {
                                        case CardDB.cardName.totemiccall: retval--; break;
                                        case CardDB.cardName.totemicslam: retval--; break;
                                        default: break;
                                    }
                                }
                            }
            ...
            public bool canplayCard(Playfield p, int manacost)
            {
                ...
                if (isRequirementInList(CardDB.ErrorType2.REQ_ENTIRE_ENTOURAGE_NOT_IN_PLAY))
                {
                    ...
                    bool haswrath = false;
                    bool hastaunt = false;
                    bool hassearing = false;
                    foreach (Minion m in p.ownMinions)
                    {
                        if (m.name == CardDB.cardName.healingtotem) hasheal = true;
                        if (m.name == CardDB.cardName.wrathofairtotem) haswrath = true;
                        if (m.name == CardDB.cardName.searingtotem) hassearing = true;
                        if (m.name == CardDB.cardName.stoneclawtotem) hastaunt = true;
                    }
        ...
        public SimTemplate getSimCard(cardIDEnum id)
        {
            switch (id)
            {
                ...
                case cardIDEnum.UNG_917e: return new Sim_UNG_917e();
                ...
                case cardIDEnum.UNG_922t1: return new Sim_UNG_922t1();
                case cardIDEnum.UNG_923: return new Sim_UNG_923();
                case cardIDEnum.UNG_925: return new Sim_UNG_925();
                case cardIDEnum.UNG_926: return new Sim_UNG_916();
        ...
        private void setAdditionalData()
        {
            ...
            foreach (Card c in this.cardlist)
            {
                ...
                if (pen.anti_aoe_minion.ContainsKey(c.name))
                {
                    c.anti_aoe_minion = pen.anti_aoe_minion[c.name];
                }

                if (pen.summonMinionSpellsDatabase.ContainsKey(c.name))
                {
