...
namespace HREngine.Bots
{
    ...
    public struct triggerCounter
    {
        ...
        public int ownSpellpowerMinionDied;
        public int enemySpellpowerMinionDied;

        public int ownJunglemoonkinDied;
        public int enemyJunglemoonkinDied;
    ...
    public class Playfield
    {
        ...
        //##########################
		public int CountEnemyAcolyteStarted = 0;
        public int CountEnemyDoomsayerStarted = 0;
        ...
        public int ownMinionsCostMore;
        public int ownMinionsCostMoreAtStart;
        ...
        public int destroyunitattack = 0;
        ...
        public static Playfield Instance
        {
            get
            {
                return instance ?? (instance = new Playfield());
            }
        }

        //Helpfunctions help = Helpfunctions.Instance;

        //simutate secrets correctly
        bool secrettrigered = false;
        ...
        public Playfield()
        {
            ...
            {
                if (mnn.name == CardDB.cardName.junglemoonkin && !mnn.silenced) spellpower += 2;
            }
        ...
        public Playfield(Playfield p)
        {
            ...
            this.CountEnemyAcolyteStarted = p.CountEnemyAcolyteStarted;
            this.CountEnemyDoomsayerStarted = p.CountEnemyDoomsayerStarted;
        ...
        public bool isEqual(Playfield p, bool logg)
        {
            ...
            for (int i = 0; i < this.ownMinions.Count; i++)
            {
                ...
                if (dis.name != pis.name) minionbool = false; minion += "name " +dis.name;
            ...
            {
                Minion dis = this.ownMinions[i]; Minion pis = p.ownMinions[i];
                if (dis.entityID != pis.entityID) Ai.Instance.updateEntitiy(pis.entityID, dis.entityID, logg);
        ...
        public override int GetHashCode()
        {
            ...
            retval -= this.enemyHero.Hp + this.enemyHero.armor;
            return retval;
        }
        ...
        public int getBestPlace(CardDB.Card card, bool lethal)
        {
            ...
            int bigmobzonepos = 1;
            int jugglercount = 0;
            bool hasbufferingminion = false;

            bool hasbufferingminioninhand = false;
            foreach (Handmanager.Handcard hcc in this.owncards)
            {
                if (hcc.card.name == CardDB.cardName.direwolfalpha || hcc.card.name == CardDB.cardName.flametonguetotem || hcc.card.name == CardDB.cardName.defenderofargus)
                {
                    hasbufferingminioninhand = true;
                }
            }

            int bufferingminionzonepos = 0;
            int jugglerzonepos = 0;
            foreach (Minion mnn in this.ownMinions)
            {
                if (mnn.name == CardDB.cardName.seagiant || mnn.name == CardDB.cardName.doomguard) bigmobzonepos = mnn.zonepos;
                if (mnn.name == CardDB.cardName.knifejuggler)
                {
                    jugglercount++;
                    jugglerzonepos = mnn.zonepos;
                }
                if (mnn.name == CardDB.cardName.direwolfalpha && !mnn.silenced || mnn.name == CardDB.cardName.flametonguetotem && !mnn.silenced)
                {
                    hasbufferingminion = true;
                    bufferingminionzonepos = mnn.zonepos;
                }
            }

            if (Ai.Instance.botBase is BehaviorAggroWarlock && !hasbufferingminion)
            {
                if (card.name == CardDB.cardName.knifejuggler) return 1;
                if (hasbufferingminioninhand && !card.Charge && !card.tank) return this.ownMinions.Count + 1;
                if (card.name == CardDB.cardName.seagiant || card.name == CardDB.cardName.doomguard)
                {
                    if (jugglercount == 0) return 1;
                    if (jugglercount >= 1) return jugglercount + 1;
                }
                if (card.name == CardDB.cardName.possessedvillager) return this.ownMinions.Count + 1;
                if (card.type == CardDB.cardtype.MOB && card.name != CardDB.cardName.defenderofargus && card.name != CardDB.cardName.direwolfalpha)
                {
                    if (bigmobzonepos >= 2) return bigmobzonepos + 1;
                    if (bigmobzonepos == 1) return jugglercount + 1;
                }
            }
            ...
            if (Ai.Instance.botBase is BehaviorAggroWarlock && hasbufferingminion)
            {
                ...
                else if (this.ownMinions.Count == 2)
                {
                    if (bufferingminionzonepos == 1) return 1;
                    else if (bufferingminionzonepos == 2) return 3;
                }
                //if (card.name == CardDB.cardName.knifejuggler) return 1;
                //if (card.deathrattle) return this.ownMinions.Count + 1;

                //if (bufferingminionzonepos > this.ownMinions.Count / 2 + 1) return this.ownMinions.Count + 1;
                //else return jugglerzonepos + 1;


                int bufferThanLeft_Count = 0;
                int bufferThanLeft_Count_Taunt = 0;
                int bufferThanRight_Count = 0;
                int bufferThanRight_Count_Taunt = 0;

                foreach (Minion mnn in this.ownMinions)
                {
                    if (mnn.zonepos < bufferingminionzonepos) bufferThanLeft_Count++;
                    if (mnn.zonepos < bufferingminionzonepos && mnn.taunt) bufferThanLeft_Count_Taunt++;
                    if (mnn.zonepos > bufferingminionzonepos) bufferThanRight_Count++;
                    if (mnn.zonepos > bufferingminionzonepos && mnn.taunt) bufferThanRight_Count_Taunt++;
                }

                int leftValuableCount = bufferThanLeft_Count - bufferThanLeft_Count_Taunt;
                int rightValueableCount = bufferThanRight_Count - bufferThanRight_Count_Taunt;

                if (leftValuableCount <= rightValueableCount) return jugglerzonepos + 1;
                if (leftValuableCount > rightValueableCount) return this.ownMinions.Count + 1;
            ...
            if (this.ownMinions.Count == 1)
            {
                foreach (Minion buff in this.ownMinions)
                {
                    if (buff.name == CardDB.cardName.direwolfalpha || buff.name == CardDB.cardName.flametonguetotem) return 1;
                }
                return 2;
            }
            if ((card.name == CardDB.cardName.tuskarrtotemic || card.name == CardDB.cardName.barnes) && this.ownMinions.Count == 1) return 1;
        ...
        public bool ownHeroHasDirectLethal()
        {
            ...
            foreach (Minion m in this.enemyMinions)
            {
                ...
                if (!m.silenced && (m.name == CardDB.cardName.ragnarosthefirelord ||
                m.name == CardDB.cardName.ancientwatcher ||
                m.name == CardDB.cardName.humongousrazorleaf ||
                m.name == CardDB.cardName.argentwatchman)) continue;

                if (!m.silenced && (m.name == CardDB.cardName.eeriestatue))
                {
                    if (this.ownMinions.Count + this.enemyMinions.Count >= 2) continue;
                }
        ...
        public void endTurn(bool simulateTwoTurns, bool playaround, bool print = false, int pprob = 0, int pprob2 = 0)
        {
                ...
                if (!this.enemyHero.immune && this.turnCounter == 0)
                {
                    ...
                    if (!eHasTaunt && !this.enemyHero.immune)
                    {
                        ...
                        this.evaluatePenality += 3 * hasReady;
                    }
                    else
                    {
                        if (!simulateTwoTurns)
        ...
        public void endEnemyTurn() //
        {
            ...
            if (!this.ownHero.immune && (this.turnCounter == 1 || this.turnCounter == 3))// enemys turn ends -> attack with all minions face (if there is no taunt)
            {
                ...
                if (!oHasTaunt && !this.ownSecretsIDList.Contains(CardDB.cardIDEnum.EX1_295)) //iceblock
                {
                    int attack = 0;
                    foreach (Minion m in this.enemyMinions)
                    {
                    ...
                    foreach (CardDB.cardIDEnum secretID in this.ownSecretsIDList)
                    {
                        ...
                        if (secretID == CardDB.cardIDEnum.EX1_130 || secretID == CardDB.cardIDEnum.EX1_594 || secretID == CardDB.cardIDEnum.EX1_533)//noble sacrifice //vaporize //misdirection
                        {
                            attack -= MINION.Angr; break;
                        }
                    }
                    ...
                    {
                        case CardDB.cardName.steadyshot: attack += 2; break; //hunter
                        case CardDB.cardName.ballistashot: attack += 3; break; //hunter 3 damage
                        case CardDB.cardName.daggermastery: if (this.enemyWeaponAttack == 0) attack += 1; break; //rogue
                        case CardDB.cardName.poisoneddaggers: if (this.enemyWeaponAttack == 0) attack += 2; break; //rogue 2 att
                        case CardDB.cardName.shapeshift: attack += 1; break; //druid
                        case CardDB.cardName.direshapeshift: attack += 2; break; //druid 2att
                        case CardDB.cardName.fireblast: attack += 1; break; //mage
                        case CardDB.cardName.fireblastrank2: attack += 2; break; //mage 2att
                        case CardDB.cardName.mindspike: attack += 2; break; //dark priest
                        case CardDB.cardName.mindshatter: attack += 3; break; //dark priest rank2
                        default : break;
                    }
        ...
        public void doAction(Action aa)
        {
            ...
            else
            {
                ...
                if (a.actionType == actionEnum.attackWithHero)
                {
                    ...
                    if (a.target != null) attackWithWeapon(a.own, a.target, a.penalty);
                    //Console.WriteLine("HERO ATTACK:::::::::::::###################################");
                }
                else
                {
        ...
        public void attackWithWeapon(Minion hero, Minion target, float penality)
        {
            bool own = hero.own;
            ...
            switch (own ? this.ownWeaponName : this.enemyWeaponName)
            {
                ...
                case CardDB.cardName.vinecleaver:
                    int pos1 = (own) ? this.ownMinions.Count : this.enemyMinions.Count;
                    this.callKid(CardDB.Instance.getCardDataFromID(CardDB.cardIDEnum.CS2_101t), pos1, own);
                    this.callKid(CardDB.Instance.getCardDataFromID(CardDB.cardIDEnum.CS2_101t), pos1, own);//silver hand recruit
                    break;
        ...
        public void playACard(Handmanager.Handcard hc, Minion target, int position, int choice, float penality)
        {
            ...
            else if (newTarget != -2) // trigger spell-secrets!
            {
        ...
        public void playHeroPower(Minion target, float penality, bool ownturn)
        {
        ...
        public void doDmgTriggers()
        {
            ...
            if (this.tempTrigger.ownMinionsDied + this.tempTrigger.enemyMinionsDied >= 1)
            {
                ...
                this.tempTrigger.ownSpellpowerMinionDied = 0;
                this.tempTrigger.enemySpellpowerMinionDied = 0;
                this.tempTrigger.ownJunglemoonkinDied = 0;
                this.tempTrigger.enemyJunglemoonkinDied = 0;
            }
        ...
        public void triggerAMinionDied()
        {
            ...
            {
                if (this.tempTrigger.ownSpellpowerMinionDied >= 1)
                {
                    bool hasspellpower = false;
                    foreach (Minion m in this.ownMinions)
                    {
                        if (m.Hp >= 1 && m.spellpower >= 1) hasspellpower = true;
                    }
                    if (!hasspellpower)
                    {
                ...
                if (this.tempTrigger.enemyJunglemoonkinDied >= 1)
                {
                    bool hasspellpower = false;
                    foreach (Minion m in this.ownMinions)
                    {
            ...
            {
                if (this.tempTrigger.enemySpellpowerMinionDied >= 1)
                {
                    bool hasspellpower = false;
                    foreach (Minion m in this.enemyMinions)
                    {
                ...
                if (this.tempTrigger.ownJunglemoonkinDied >= 1)
                {
                    bool hasspellpower = false;
                    foreach (Minion m in this.enemyMinions)
                    {
        ...
        public void triggerACardWillBePlayed(Handmanager.Handcard hc, bool own, Minion target, int choice)
        {
            ...
            if (own)
            {
                ...
                for (int i = 0; i < wickedwitchdoctor; i++)
                {
                    int pos = this.ownMinions.Count;
                    if (this.ownHero.Hp <= 10)
                    {
                        this.callKid(CardDB.Instance.getCardDataFromID(CardDB.cardIDEnum.CS2_051), pos, own); //taunt totem
                    }
                    else this.callKid(CardDB.Instance.getCardDataFromID(CardDB.cardIDEnum.GVG_039), pos, own); // vutality totem
                }
            ...
            else
            {
                ...
                {
                    int pos = this.enemyMinions.Count;
                    this.callKid(CardDB.Instance.getCardDataFromID(CardDB.cardIDEnum.NEW1_009), pos, own); //healing
                }
                for (int i = 0; i < illidan; i++)
                {
                    int pos = this.enemyMinions.Count;
        ...
        public void triggerAMinionIsSummoned(Minion m, bool callkid = false)
        {
            if (m.own)
            {
                ...
                if (this.ownWeaponName == CardDB.cardName.spiritclaws)
                {
                    if (this.spellpower >= 1) return;
                    if (m.handcard.card.Spellpower)
                    {
                        this.ownWeaponAttack += 2;
                        this.minionGetBuffed(this.ownHero, 2, 0);
                    }
                }
                if (m.name == CardDB.cardName.junglemoonkin)
                {
                    if (this.enemyWeaponName == CardDB.cardName.spiritclaws)
                    {
                        if (this.enemyspellpower >= 1) return;
                        if (m.handcard.card.Spellpower)
                        {
                            this.enemyWeaponAttack += 2;
                            this.minionGetBuffed(this.enemyHero, 2, 0);
                ...
                if (this.enemyMinions.Find(a => a.name == CardDB.cardName.swampkingdred && !a.silenced) != null && callkid == false)
                {
                    Minion attacker;
                    Minion deffender = m;
                    foreach (Minion ATK in this.enemyMinions)
                    {
                        if (ATK.name == CardDB.cardName.swampkingdred && !ATK.silenced)
                        {
                            attacker = ATK;
                            this.minionAttacksMinion(attacker, deffender, false);
                            if (attacker.Angr >= deffender.Hp && !deffender.divineshild ||
                                deffender.Angr >= attacker.Hp && !attacker.divineshild ||
                                attacker.poisonous && !deffender.divineshild ||
                                deffender.poisonous && !attacker.divineshild) break; //when 2+ swampkingdred & died first action
                        }                       
                    }                    
                }
        ...
        public void triggerStartTurn(bool ownturn)
        {
            ...
            if (this.ownWeaponName == CardDB.cardName.unknown || this.ownWeaponDurability == 0) this.ownHero.poisonous = false; // breaked weapon when enemy turns.. 
            ...
            foreach (Minion m in ownm)
            {
                ...
                {
                    this.destroyunitattack = m.Angr;
                    //Helpfunctions.Instance.ErrorLog("destroyunitattack = " + BehaviorAggroWarlock.Instance.destroyunitattack);
                    //this.minionGetDestroyed(m);
                }
        ...
        public void changeRecall(bool own, int value)
        {
            ...
            if (oldrecall < newrecall)
            {
                foreach (Minion m in tempminions)
                {
                    ...
                    if (!m.silenced && m.name == CardDB.cardName.unboundelemental)
                    {
                        this.minionGetBuffed(m, 1, 1);
                    }
                }
        ...
        public void secretTrigger_HeroPowerUsed(bool own)
        {
            ...
            if (turnCounter == 0)
            {
                //this.evaluatePenality -= triggered * 2;
            }
        ...
        public void doDeathrattles(List<Minion> deathrattles)
        {
            ...
            foreach (Minion m in deathrattles)
            {
                ...
                for (int i = 0; i < m.spikeridgedteed; i++)
                {
                ...
                {
                    CardToHand(CardDB.cardName.unknown, m.own);
                }
            ...
            else
            {
                if (s == CardDB.cardName.unknown && !nopen) 
                {
                    ...
                    else
                    {
                        ...
                        {
                            this.evaluatePenality -= (this.turnCounter > 2) ? 2 : 5;
                            return;
                        }
                        this.enemycarddraw++;
                        this.enemyAnzCards++;
        ...
        public void printBoard(bool justactions = false, int boardnumber = -1)
        {
            ...
            else
            {
                ...
                {
                    //string secs = "";
                    //foreach (CardDB.cardName sec in this.ownSecretsIDList)
                    //{
                    //    secs += sec + " ";
                    //}
                    //Helpfunctions.Instance.logg("own secrets: " + secs + "\r\n");
                    string secID = "";
        ...
        public string getCompleteBoardForSimulating(String settings, String version, String time)
        {
            ...
            */
            ...
            foreach (Handmanager.Handcard c in this.owncards)
            {
                ...
                data +="pos " + c.position + " " + c.card.name + " " + c.getManaCost(this) + " entity " + c.entity + " " + c.card.cardIDenum + " " + c.addattack+ " " + c.addHp + " " + c.elemPoweredUp + "\r\n";
            }
        ...
        public string getMinionString(Minion m)
        {
            ...
            if (m.spikeridgedteed >= 1) mini += " spikeridged(" + m.spikeridgedteed + ")";
