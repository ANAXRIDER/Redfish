namespace HREngine.Bots
{
    ...
    public class SecretItem
    {
        ...
        public bool canBe_missdirection = false; // few person use this.
        ...
        public bool canBe_eyeforaneye = false; //because no one use e f e
        ...
        public static SecretItem instance = new SecretItem();

        public static SecretItem Instance
        {
            get
            {
                return instance;
        ...
        public int iceblockmage;

        private bool Iceblockmage()
        {
            bool ret = false;
            foreach (KeyValuePair<CardDB.cardIDEnum, int> e in Probabilitymaker.Instance.enemyGraveyard)
            {
                if (e.Key == CardDB.cardIDEnum.NEW1_021) iceblockmage += 2; //doomsayer NEW1_021
                if (e.Key == CardDB.cardIDEnum.CS2_026) iceblockmage += 2; //frostnova CS2_026
        ...
        public SecretItem(SecretItem sec)
        {
            ...
            if (Playfield.Instance.enemyHeroName == HeroEnum.mage)
            {
                Iceblockmage();
        ...
        public void usedTrigger_CharIsAttacked(bool DefenderIsHero, bool AttackerIsHero)
        {
            if (DefenderIsHero)
            {
                //foreach (SecretItem s in Probabilitymaker.Instance.enemySecrets)
                //{
        ...
        public void usedTrigger_MinionIsPlayed(int numberMinionsOnBoard)
        {
            ...
            //    if (numberMinionsOnBoard >= 3) s.canBe_Trial = false;
            //}
        ...
        public void usedTrigger_SpellIsPlayed(bool minionIsTarget)
        {
            ...
            //    if (minionIsTarget) s.canBe_spellbender = false;
            //}
            this.canBe_counterspell = false;
            this.canBe_cattrick = false;
        ...
        public void usedTrigger_HeroGotDmg(bool deadly = false)
        {
            ...
            //    if (deadly) s.canBe_iceblock = false;
            //}
    ...
    public class Probabilitymaker
    {
        ...
        public SecretItem getNewSecretGuessedItem(int entityid, HeroEnum enemyHeroName)
        {
            ...
            if (enemyHeroName == HeroEnum.priest ||
                enemyHeroName == HeroEnum.thief) //steal
            {
                if (Playfield.Instance.ownHeroName == HeroEnum.mage)
                {
                    sec.canBe_snaketrap = false;
                    sec.canBe_snipe = true;
                    sec.canBe_explosive = true;
                    sec.canBe_freezing = false;
                    sec.canBe_missdirection = false;

                    sec.canBe_counterspell = true;
                    sec.canBe_icebarrier = true;
                    sec.canBe_iceblock = true;
                    sec.canBe_mirrorentity = true;
                    sec.canBe_spellbender = true;
                    sec.canBe_vaporize = true;
                    sec.canBe_duplicate = true;
                ...
                else if (Playfield.Instance.ownHeroName == HeroEnum.hunter)
                {
                    sec.canBe_snaketrap = true;
                    sec.canBe_snipe = true;
                    sec.canBe_explosive = true;
                    sec.canBe_freezing = true;
                    sec.canBe_missdirection = true;
                    ...
                    sec.canBe_effigy = false;
                    sec.canBe_beartrap = true;
                    sec.canBe_competivespirit = false;

                    sec.canBe_Trial = false;
                    sec.canBe_Dart = true;

                    sec.canBe_cattrick = true;
                }
                else if (Playfield.Instance.ownHeroName == HeroEnum.pala)
                {
                    sec.canBe_snaketrap = false;
                    sec.canBe_snipe = true;
                    sec.canBe_explosive = true;
                    ...
                    sec.canBe_eyeforaneye = true;
                    sec.canBe_noblesacrifice = true;
                    sec.canBe_redemption = true;
                    sec.canBe_repentance = true;
                    sec.canBe_avenge = true;

                    sec.canBe_effigy = false;
                    sec.canBe_beartrap = false;
                    sec.canBe_competivespirit = true;

                    sec.canBe_Trial = true;
                    sec.canBe_Dart = false;
            ...
            if (enemyHeroName == HeroEnum.druid ||
                ...
                enemyHeroName == HeroEnum.warlock ||
                enemyHeroName == HeroEnum.shaman) //can't simulate youggsaron's secret.. just do best play
            {
                sec.canBe_snaketrap = false;
                sec.canBe_snipe = true;
                sec.canBe_explosive = true;
